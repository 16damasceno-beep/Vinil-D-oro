...
export const Navbar: React.FC = () => {
  ...
  const handleAction = (e: React.MouseEvent, action: 'APPROVE' | 'REJECT', reservationId?: string) => {
    ...
    if (action === 'APPROVE') approveReservation(reservationId);
    else rejectReservation(reservationId);
  };
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            ...
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
                ...
                <Link to="/procuro-por" className={`px-3 py-2 rounded-md text-sm font-medium ${isActive('/procuro-por')}`}>Procuro Por</Link>
                {currentUser && (
          ...
          <div className="hidden md:block">
            <div className="ml-4 flex items-center md:ml-6 gap-4">
              <button onClick={handleShareApp} className="text-gray-400 hover:text-vinyl-accent relative group" title="Compartilhar">
              ...
              {currentUser ? (
                <>
                  <Link to="/favorites" className="text-gray-400 hover:text-red-500 relative group" title="Favoritos">
                    ...
                    {favoritesCount > 0 && <span className="absolute -top-1 -right-1 bg-gray-700 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{favoritesCount}</span>}
                  </Link>
                  <div className="relative" ref={notificationRef}>
                    <button onClick={handleNotificationClick} className="text-gray-400 hover:text-white relative focus:outline-none">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                      </svg>
                      {unreadCount > 0 && <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center animate-pulse">{unreadCount}</span>}
                    </button>
                    {showNotifications && (
                      <div className="absolute right-0 mt-2 w-96 bg-gray-900 border border-gray-700 rounded-md shadow-2xl overflow-hidden z-50">
                        <div className="p-3 border-b border-gray-700 font-bold text-white bg-gray-800">Notificações</div>
                          ...
                          {(!currentUser.notifications || currentUser.notifications.length === 0) ? (
                            ...
                            currentUser.notifications.slice(0, 10).map(n => (
                              <div key={n.id} className={`p-4 border-b border-gray-800 hover:bg-gray-800 ${!n.read ? 'bg-gray-800/50' : ''}`}>
                                ...
                                {n.type === 'WANT_RESPONSE' && n.metadata?.wantRequestId && (
                                   <Link to={`/procuro-por/${n.metadata.wantRequestId}`} className="text-[10px] text-vinyl-accent font-bold mt-1 block">Ver Respostas</Link>
                                )}
                              </div>
                            ))
                          )}
                  ...
                  <div className="flex items-center gap-4 border-l border-gray-700 pl-4">
                    <Link to="/profile" className={`px-3 py-2 rounded-md text-sm font-medium bg-gray-800 hover:bg-gray-700 ${isActive('/profile')}`}>Meu Perfil</Link>
                    <button onClick={logout} className="text-sm text-gray-400 hover:text-white">Sair</button>
                  </div>
                </>
              ) : (
                 <Link to="/login" className="bg-vinyl-accent hover:bg-yellow-600 text-black px-4 py-2 rounded-md text-sm font-bold">Entrar</Link>
              )}
            </div>
          </div>
          <div className="-mr-2 flex md:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="bg-gray-800 inline-flex items-center justify-center p-2 rounded-md text-gray-400">
